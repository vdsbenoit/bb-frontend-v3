      ...
      <ion-grid class="ion-no-padding" v-if="sectionId">
        <ion-row>
              ...
              <ion-card-content>
                <ion-list v-if="sectionMembers && sectionMembers.size > 0">
                  <ion-item v-for="member in sectionMembers.values()" :key="member.uid" :routerLink="`/profile/${member.uid}`">
                    ...
                    <ion-badge slot="end" :color="registrationStatus(member).color">{{ registrationStatus(member).text }}</ion-badge>
                  </ion-item>
                </ion-list> 
                <div v-else-if="isLoadingSection" class="ion-text-center ion-align-items-center">
                  <ion-spinner></ion-spinner>
          ...
          <ion-col  size="12" size-sm="6">
            <ion-card>
              <ion-card-header>
                <ion-card-title>Chefs</ion-card-title>
              </ion-card-header>
              <ion-card-content>
                <ion-list v-if="sectionLeaders && sectionLeaders.length > 0">
                  <ion-item v-for="leader in sectionLeaders" :key="leader.uid" :routerLink="`/profile/${leader.uid}`">
                    <ion-label>{{ user.getName(leader.uid) }}</ion-label>
                    <ion-badge slot="end" :color="registrationStatus(leader).color">{{ registrationStatus(leader).text }}</ion-badge>
                  </ion-item>
                </ion-list>
                <div v-else-if="isLoadingSection" class="ion-text-center ion-align-items-center">
                  <ion-spinner></ion-spinner>
...
import { getLeaderSection, getLeaderSections, LeaderSection } from "@/services/leaderSections";
...
onMounted(() => {
  if (route.params.sectionId) {
    sectionId.value = route.params.sectionId as string;
  }
  setTimeout(() => {
...
const sectionMembers = computed(() => {
  return user.getLeaderSectionMembers(sectionId.value);
});
const sectionLeaders = computed(() => {
  return Array.from(sectionMembers.value.values()).filter((user) => user.role === ROLES.Chef);
});
