...
export const setMorningLeader = async (game: RefGame, uid = "") => {
  ...
  if ((game.value.morningLeaders.length as number) >= maxGameLeaders)
    throw new Error(`Le nombre maximum d'animateurs a été atteint pour l'épreuve ${gameId} au matin`)
...
export const setAfternoonLeader = async (game: RefGame, uid = "") => {
  ...
  const maxGameLeaders = await getMaxGameLeaders()
  if ((game.value.afternoonLeaders.length as number) >= maxGameLeaders)
    throw new Error(`Le nombre maximum d'animateurs a été atteint pour l'épreuve ${gameId} l'après-midi`)
